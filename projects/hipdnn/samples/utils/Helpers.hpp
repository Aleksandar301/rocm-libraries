...
#define HIPDNN_CHECK(status) \
    ...
    { \
        ...
        { \
            std::cerr << "hipDNN Error: " << hipdnnGetErrorString(status) \
                      << " in file " << __FILE__ \
                      << " at line " << __LINE__ << std::endl; \
            exit(EXIT_FAILURE); \
        } \
...
#define HIPDNN_FE_CHECK(statusObj) \
    ...
    { \
        ...
        if(!status.is_good()) \
        { \
            std::cerr << "hipDNN Frontend Error: " << status.get_message() \
                      << " in file " << __FILE__ \
                      << " at line " << __LINE__ << std::endl; \
            exit(EXIT_FAILURE); \
        } \
             ...
             const TensorLayout& layout = TensorLayout::NCHW)
{
    ...
    tensor->set_dim(dims)
          ...
          .set_stride(hipdnn_data_sdk::utilities::generateStrides(dims, layout.strideOrder));
    return tensor;
}
