...
bool SampleRunner::operator()(const TensorLayout& layout)
{
    ...
    std::cout << "Running convolution backward data graph " << inputType
              << " [" << layout << "]"
    ...
    // Apply engine override if user provided one
    ...
    if (config.cpuValidation)
    {
        ...
        auto absoluteTolerance =
            hipdnn_test_sdk::utilities::conv::calculateConvDgradTolerance<
                InputType, InputType, float>(0.0, 1.0, 0.0, 1.0, wAttr->get_dim());

        constexpr float relativeTolerance = 0.01f;

        auto dxValidator =
            hipdnn_test_sdk::utilities::CpuFpReferenceValidation<InputType>(
                absoluteTolerance, relativeTolerance);

        std::cout << "CPU reference validation:\n";
        bool dxValid =
            hipdnn_test_sdk::utilities::validateAndReport<InputType>(
                std::cout,
                "dx",
                dxValidator,
                dxRefTensor,
    ...
    std::cout << "Convolution backward data graph execution complete for "
              << inputType << ".\n\n";
